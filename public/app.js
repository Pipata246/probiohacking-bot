...
document.addEventListener('click', (e) => {
  ...
  // Виджет запросов - НОВЫЕ КЛАССЫ БЕЗ КОНФЛИКТОВ
  if (e.target.closest('.query-card')) {
    const card = e.target.closest('.query-card');
    const queryIndex = parseInt(card.getAttribute('data-query'));
    ...
    if (queryIndex >= 0 && queryIndex < allQuestions.length) {
      openChatWithMessage(allQuestions[queryIndex]);
    }
    return;
  }
