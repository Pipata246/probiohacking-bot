...
- Формат: 2-5 предложений или короткий список`;
      ...
      // 📋 DETAILED MODE – полный ответ, НО БЕЗ СОЗДАНИЯ ПРОГРАММЫ И ДНЕВНИКА
      systemPrompt += `\n\n📋 ПОДРОБНЫЙ ОТВЕТ БЕЗ ПРОГРАММЫ: Ответь на вопрос пользователя полностью и информативно. Будь точен и лаконичен - избегай воды и лишних повторений. Используй его личные данные, анализы и результаты опросов ТОЛЬКО если они релевантны его вопросу. НЕ анализируй здоровье если его об этом не просили.
...
Сделай качественный экспертный ответ и мягко порекомендуй пройти диагностику и/или загрузить анализы, если это уместно по смыслу вопроса.`;
    ...
    const apiReq = https.request({
      ...
      apiRes.on('end', async () => {
        ...
        // Раньше здесь извлекалась структурированная программа (health + diary) из ответа.
        // Теперь чат НЕ создаёт программу: просто используем полный текст ответа.
        const healthProgram = null;
        const diaryEntries = [];
        ...
        const canCreateProgram = false; // Чат больше не создаёт программу
