...
module.exports = async (req, res) => {
  ...
  try {
    ...
    // Флаги статусов диагностики и анализов (нужны для управления режимом ответа)
    ...
    const canUseProgramMode = quizCompletedFlag && analysesUploadedFlag;
...
- Формат: 2-5 предложений или короткий список`;
      ...
      // 📋 DETAILED MODE
      if (canUseProgramMode) {
        // Полный режим: диагностика пройдена и анализы загружены — можно составлять программу
...
Не добавляй комментариев вокруг JSON и не вставляй туда лишний текст.`;
        ...
        // Ограниченный режим: диагностика или анализы НЕ завершены — программу и JSON составлять нельзя
        systemPrompt += `\n\n📋 ПОДРОБНЫЙ ОТВЕТ БЕЗ ПРОГРАММЫ: Ответь на вопрос пользователя полностью и информативно. Будь точен и лаконичен - избегай воды и лишних повторений. Можешь использовать имеющиеся данные (анкета, частичные анализы), НО:
⚠️ НЕ СОСТАВЛЯЙ персонализированную программу на месяц и НЕ описывай подробный распорядок на 30 дней.
⚠️ НЕ выводи никакой JSON, НЕ используй маркеры === STRUCTURED_PROGRAM_JSON_START === / === STRUCTURED_PROGRAM_JSON_END === и НЕ формируй массив diary.
Сделай качественный экспертный ответ и мягко порекомендуй пройти диагностику и/или загрузить анализы, если это уместно по смыслу вопроса.`;
      }
    ...
    const apiReq = https.request({
      ...
      apiRes.on('end', async () => {
        ...
        let { healthProgram, diaryEntries, cleanedText } = extractProgramFromContent(fullContent);
        ...
        // Если хотя бы один статус (диагностика или анализы) = false,
        // НЕ передаём на фронт структуру программы и дневник, даже если модель их сгенерировала
        if (!quizCompletedFlag || !analysesUploadedFlag) {
          healthProgram = null;
          diaryEntries = [];
        }
